...
function AlertsPage() {
        ...
        try {
          ...
          if (Array.isArray(parsed)) {
            return parsed.map((a: any) => {
              const initMatch = INITIAL_ALERTS.find((i) => i.id === a.id);
              const catMatch = CATEGORIES.find((c) => c.id === a.category);
              return {
                ...a,
                icon: initMatch?.icon || catMatch?.icon || Bell,
              };
            });
          }
        } catch (e) {}
  ...
  useEffect(() => {
    localStorage.setItem("aks.alerts", JSON.stringify(alerts));
    window.dispatchEvent(new Event("aks-alerts-changed"));
  }, [alerts]);
...
function AlertCard({
  ...
  const cat = CATEGORIES.find((c) => c.id === alert.category) || CATEGORIES[0];
  const sev = SEVERITY_META[alert.severity] || SEVERITY_META.medium;
  const st = STATUS_META[alert.status] || STATUS_META.active;
  const Icon = alert.icon || cat.icon || Bell;
  const StatusIcon = st.icon || Bell;
