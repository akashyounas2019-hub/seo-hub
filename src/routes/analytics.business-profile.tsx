...
function BusinessProfileDrilldown() {
  return (
    <div className="min-h-screen bg-[#05070d] text-slate-200">
      <div className="mx-auto max-w-7xl px-6 py-8">
        ...
        <div className="mt-4 flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="text-[10px] font-medium uppercase tracking-[0.2em] text-fuchsia-300/80">
              Google Business Profile
            </div>
            <h1 className="mt-1 text-2xl font-semibold text-white">Local Presence Insights</h1>
            <p className="mt-1 text-sm text-slate-400">
              Last 28 days · AKS Cleaning Services — Dubai
            </p>
          </div>
            ...
            <div className="mt-4 overflow-hidden rounded-lg border border-slate-800">
              <table className="w-full text-left text-xs">
                ...
                <tbody className="divide-y divide-slate-800">
                  {QUERIES.map((q) => (
                    <tr key={q.q} className="hover:bg-slate-900/60">
                ...
                {PHOTOS.map((p) => (
                  <div key={p.name} className="flex items-center justify-between">
                    ...
                    <span className="tabular-nums text-slate-400">
                      {p.views.toLocaleString()} <span className="text-emerald-300">+{p.delta}%</span>
                    </span>
                  </div>
                ))}
              </div>
        ...
        <section className="mt-6 rounded-2xl border border-slate-800 bg-slate-900/40 p-5">
          ...
          <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2">
            {REVIEWS.map((r) => (
              <div key={r.author} className="rounded-lg border border-slate-800 bg-slate-950/40 p-3">
                <div className="flex items-center justify-between">
