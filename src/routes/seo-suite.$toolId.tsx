      ...
      <div className="relative mx-auto max-w-7xl px-6 py-8">
        ...
        <div className="mt-6 grid gap-4 lg:grid-cols-3">
          ...
          <aside className="rounded-2xl border border-slate-800 bg-slate-950/50 p-5">
            ...
            <div className="mt-2 text-sm font-semibold text-white">
              {running ? "Scout is analysing…" : ranAt ? "Last run complete" : "Idle"}
            </div>
            ...
            <button className="mt-5 inline-flex w-full items-center justify-center gap-1.5 rounded-lg border border-slate-800 bg-slate-950/60 px-3 py-2 text-[11px] text-slate-400 hover:border-cyan-400/30 hover:text-cyan-200">
        ...
        <section className="mt-4 rounded-2xl border border-slate-800 bg-slate-950/50 p-5">
          <div className="flex items-center justify-between">
            ...
            <span className="inline-flex items-center gap-1.5 rounded-full border border-slate-700 bg-slate-900/60 px-2 py-0.5 text-[10px] uppercase tracking-wider text-slate-300">
              <Sparkles className="h-3 w-3 text-cyan-300" /> Sample
            </span>
          </div>
          <pre className="mt-4 max-h-72 overflow-auto rounded-xl border border-slate-800 bg-black/40 p-4 text-[12px] leading-relaxed text-slate-300">
{`# ${tool.title}
> ${tool.tagline}

## Summary
- Vertical: Dubai cleaning services
- Locale: EN + AR
- Category: ${SEO_CATEGORIES[tool.category].label}

## Top findings
1. [High] Add LocalBusiness schema to /villa-cleaning
2. [Med]  Compress hero image (from 812KB → 118KB)
3. [Med]  Publish Arabic mirror for /deep-clean

## Recommendations
- Rewrite title to include "Dubai" + primary service
- Add FAQ block targeting long-tail intents
- Interlink to nearest neighbourhood landing pages`}
          </pre>
        </section>
