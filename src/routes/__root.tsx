...
import { Globe, Search, BarChart3, Zap, Link2, CheckCircle2, AlertCircle, ExternalLink, X, BookOpen, MessageSquareQuote, LayoutTemplate, ChevronDown, FolderOpen, MapPin } from "lucide-react";
...
import { MasterAuthGate } from "@/components/master-auth-gate";
import { SiteProvider, useSite } from "@/lib/site-context";
import { LogOut, Lock } from "lucide-react";

function MasterLogoutButton() {
  const { logout } = useAuth();
  return (
    <button
      onClick={logout}
      title="Lock Console (Logout)"
      className="inline-flex items-center gap-1 rounded-full border border-rose-500/30 bg-rose-500/10 px-2.5 py-1 text-[11px] font-semibold text-rose-300 hover:bg-rose-500/20 transition cursor-pointer"
    >
...
function ConnectedSitesHeaderButton() {
  ...
  return (
    ...
    >
      ...
      Connected Sites
      <span className="ml-1 rounded-full bg-cyan-400/20 px-1.5 text-[10px] font-semibold text-cyan-100">{allSites.length}</span>
    </Link>
  );
}
...
function ConnectedSitesMenu() {
  ...
  const sitesList = allSites.length > 0 ? allSites : CONNECTED_SITES;
  const totalSites = sitesList.length;
  const totalIntegrations = sitesList.reduce((a, s) => a + (s.integrations?.length || 0), 0);
  const connectedIntegrations = sitesList.reduce(
    (a, s) => a + (s.integrations?.filter((i) => i.status === "connected").length || 0),
    0
  );
  const needsAttention = sitesList.reduce(
    (a, s) => a + (s.integrations?.filter((i) => i.status !== "connected").length || 0),
    0
  );
